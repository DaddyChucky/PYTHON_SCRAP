...
scene = canvas(title="Simulation d'un jeu d'échecs", width=1700, height=800, center=vector(-0.5,1,0), background=color.white)

def initiateBoard(side):
    for i in range(-4,4):
        for j in range(-4,4):
            if abs(i+j) % 2 == 0: # Start white
                if side == "white":
                    box(pos = vector(i,0,j), length=1, height=0.2, width=1,color=color.white)
                else:
                    box(pos = vector(i,0,j), length=1, height=0.2, width=1,color=color.black)
            else:
                if side == "white":
                    box(pos = vector(i,0,j), length=1, height=0.2, width=1,color=color.black)
                else:
...
tile1 = box(pos = vector(-4,0,-4), length=1, height=0.2, width=1,color=color.white)
tile2 = box(pos = vector(-4,0,-3), length=1, height=0.2, width=1,color=color.black)

board = box(pos = vector(0,0,0), length=8, height=0.2, width=8,color=color.black)
